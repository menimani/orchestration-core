...
describe('buildPrBody', () => {
  it('marks an empty branch with the marker, all sections and - None', () => {
    const body = buildPrBody(repoRoot, baseRef, [])
...
describe('prTitle', () => {
  ...
  it('reports cycle progress while cycles run', () => {
    expect(prTitle(repoRoot, baseRef, 'cycle', { cycle: 3, maxCycles: 12 }))
  ...
  it('summarises the diff by category once finished, without the tooling count', () => {
    ...
    expect(prTitle(repoRoot, baseRef, 'final', { cycle: 3, maxCycles: 12 }))
...
describe('prRisks', () => {
  it('reports None identified for a branch with no risk signals', () => {
    ...
    expect(prRisks(repoRoot, baseRef, [])).toBe('- None identified\n')
  })

  it('fences issue-number-like references in decisions', () => {
    const risks = prRisks(repoRoot, baseRef, ['Dependabot alert #1 stays open pending a major bump'])
  ...
  it('detects a migration and deleted tests', () => {
    ...
    const risks = prRisks(repoRoot, baseRef, [])
